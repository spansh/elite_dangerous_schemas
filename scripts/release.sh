...
#
# Format:
#   OpenAPI input | base JSON Schema | output JSON Schema
#
declare -a SCHEMA_BUILDS=(
  "galaxy.downloads.openapi.json|galaxy.schema.json|galaxy.schema.json"
  "merged.downloads.schema.json|galaxy.schema.json|merged.schema.json"
  "systems.downloads.openapi.json|systems.schema.json|systems.schema.json"
  "factions.downloads.openapi.json|factions.schema.json|factions.schema.json"
)
...
then
  ...
  >&2 echo "Use --skip-clean-check to include them in the release worktree."
  exit 1
fi
...
restore_original_changes() {
  ...
  if git -C "${REPO_ROOT}" stash apply --index "${STASH_COMMIT}"; then
    local stash_ref
...
cleanup() {
  ...
  if [[ "${WORKTREE_CREATED}" == true ]]; then
    git -C "${REPO_ROOT}" worktree remove \
      --force "${WORKTREE_DIR}" >/dev/null 2>&1 || true
  ...
  then
    git -C "${REPO_ROOT}" tag --delete "${NEW_TAG}" \
      >/dev/null 2>&1 || true
  fi
...
if [[ "${STASH_CREATED}" == true ]]; then
  echo "Applying uncommitted changes to temporary worktree..."
  git stash apply --index "${STASH_COMMIT}"
fi

#
# Check README after applying the stash so uncommitted README changes are
...
#
# Regenerate all JSON Schema files.
#
...
do
  ...
  generated_file="${output_schema_file}.new"

  echo "Generating ${output_schema_file}"
  echo "  Input: ${openapi_file}"
  echo "  Base : ${base_schema_file}"
  ...
  mv "${generated_file}" "${output_schema_file}"
done
...
#
# Stage only the release artifacts.
#
